...
@app.get("/api/health")
def health_check():
    caption_service_url = os.getenv("CAPTION_SERVICE_URL", "").strip()
    if caption_service_url and not caption_service_url.startswith(("http://", "https://")):
        caption_service_url = f"http://{caption_service_url}"
    caption_service = {"configured": bool(caption_service_url), "status": "skipped"}

    if caption_service_url:
        try:
            import requests

            headers = {}
            secret = os.getenv("CAPTION_SERVICE_SECRET", "").strip()
            if secret:
                headers["X-Service-Token"] = secret
            response = requests.get(
                f"{caption_service_url.rstrip('/')}/health",
                headers=headers,
                timeout=5,
            )
            caption_service = {
    ...
    return {
        "status": "ok",
        "database": "postgresql" if os.getenv("DATABASE_URL", "").startswith(("postgres://", "postgresql://")) else "sqlite",
